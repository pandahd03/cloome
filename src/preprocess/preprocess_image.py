import sys
sys.path.insert(1, r'C:\Users\Jakob S\AI-Studium\5 Semester\Practical_Work\cloome')
...
from src.clip.helpers import parallelize
...
def sixteen_to_eight_bit(arr, display_max, display_min=0):
    #apply threshold and convert to int 8
...
def group_samples(indir):
    ...
    basenames = [os.path.basename(d) for d in dirlist]  #get all basenames

    #groupe by plate ids example (26794)
    ...
    #get folder names sorted in plate_ids
    ...
    for i, plate in enumerate(fullpath_groups):
        ...
        #groupe by well inforamtion (cdp2w13_"""a01_s1"""_w1837deac0-f442-4e08-a66d-50f23afed437.tif)
        #same for each dimension of same molecule
        sample_groups = [list(g) for _, g in itertools.groupby(sorted(plate_files, key=lambda x: x[-49:-43]), lambda x: x[-49:-43])]

        #change order of each dimesnions and save 
...
def vis_images(imglist, indir):
    ...
    labels = ["Mitochondria", "Endoplasmic reticulum", "Nucleoli, cytoplasmic RNA", "Actin, Golgi, plasma membrane", "Nucleus"]

    for i in range(5):
        axs[i].imshow(sample[:,:,i],cmap="gray")
        axs[i].set_axis_off()
        axs[i].set_title(labels[i], size=17)

    plt.tight_layout()
    plt.savefig("example.png")

def process_sample(imglst, indir, outdir="."):
    #process parallelized so we get just one list as input

    #empty array for each channel of image
    sample = np.zeros((520, 696, 5), dtype=np.uint8)

    #get info about well and sampleid for naming purposes of output file
    refimg = imglst[0]
...
if __name__ == '__main__':
    indir = r"C:\Users\Jakob S\AI-Studium\5 Semester\Practical_Work\paper-bray2017\data"
    outdir = r"C:\Users\Jakob S\AI-Studium\5 Semester\Practical_Work\paper-bray2017\npzs"
    ...
    index=0
    #get list of list with each list being the 5 picture file names of one molecule
    sample_groups = group_samples(indir)
    #print(len(sample_groups))
    #print(sample_groups[index])
    #vis_images(sample_groups[index], indir)
